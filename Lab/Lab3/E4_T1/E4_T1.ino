...
PinChangeInterrupt reference code: https://github.com/NicoHood/PinChangeInterrupt/blob/master/examples/PinChangeInterrupt_Led/PinChangeInterrupt_Led.ino

Source code reference: <my name/> code for Lab 2 - Excercise 3
*/
...
byte colPins[COLS] = { 5, 4, 3, 2 }; // 4 - 5 ~ 1 - 2
byte rowPins[ROWS] = { A3, A2, A1, A0 }; // 8 - A3 ~ 5 - A0
...
// ============= ISR ==============
void magneticISR() {
  // It's reasonable to put the magnetic sensor to become
  // the first interrupt-based implementation.
  // TODO: Make this work
  tamper = 1;
  tTamper = millis();
}
...
void setup() {
  ...
  // attachInterrupt(digitalPinToInterrupt(MAGNETIC), magneticISR, RISING);
  attachPCINT(digitalPinToPCINT(MAGNETIC), magneticISR, RISING);
...
void blinkingRed() {
  if (tBlink == 0) tBlink = millis();  // set starting time stamp
...
void loop() {
  ...
  // if (tamperDetection == 0 && tCurr - tTamper >= 5000) {
  if (tamper && tCurr - tTamper >= 5000) {
    tamper = 0;
    tTamper = -1;
  }
